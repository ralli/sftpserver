...
public final class PublicKeyReaderUtil {
	...
	public static String encodeToOpenSSH(PublicKey key) throws Exception {
		StringWriter stringWriter = new StringWriter();
		if (key instanceof RSAPublicKey) {
			stringWriter.write(SSH2_RSA_KEY);
			stringWriter.append(' ');
			stringWriter.write(Base64.encodeBase64String(encodePublicKey((RSAPublicKey) key)));
		} else if (key instanceof DSAPublicKey) {
			stringWriter.write(SSH2_DSA_KEY);
			stringWriter.append(' ');
			stringWriter.write(Base64.encodeBase64String(encodePublicKey((DSAPublicKey) key)));
		}
	...
	private static byte[] getFingerPrint(PublicKey key) throws Exception {
		...
		if (key instanceof RSAPublicKey) {
			...
			throw new PublicKeyParseException(
					PublicKeyParseException.ErrorCode.UNKNOWN_PUBLIC_KEY_CLASS);
		}
	...
	public static PublicKey load(final String _key)
		...
		final SSH2DataBuffer buf = new SSH2DataBuffer(
	...
	private static PublicKey decodeDSAPublicKey(final SSH2DataBuffer _buffer)
		...
		try {
			...
			throw new PublicKeyParseException(
					PublicKeyParseException.ErrorCode.SSH2DSA_ERROR_DECODING_PUBLIC_KEY_BLOB,
					e);
		}
	}
	...
	private static PublicKey decodePublicKey(final SSH2DataBuffer _buffer)
		...
		try {
			...
			throw new PublicKeyParseException(
					PublicKeyParseException.ErrorCode.SSH2RSA_ERROR_DECODING_PUBLIC_KEY_BLOB,
					ex);
		}
	}
	 ...
	 * formatting defined in <a
	 * href="http://tools.ietf.org/html/rfc4253#section-6.6">RFC 4253 Section
	 * 6.6</a>.
	 * 
	 * @see <a href="http://tools.ietf.org/html/rfc4253#section-6.6">RFC 4253
	 *      Section 6.6</a>
	...
	private static class SSH2DataBuffer {
		 ...
		 * string is stored as byte array (see {@link #readByteArray()}) in UTF8
		 * format.
		 * 
		 * @return read string
		 * @throws PublicKeyParseException
		...
		/**
		 * Reads from the {@link #data} starting with {@link #pos} a byte array.
	...
	public static final class PublicKeyParseException extends Exception {
		...
		public enum ErrorCode {
			...
			/**
			 * The binary blob of the key definition used a not supported public
			 * key certificate format. Only DSA and RSA are currently supported.
			 * 
